...
int main(void) {
    ...
    printf("--- Cache invariance (cold state vs warm state) ---\n");
    {
        ...
        /* Only the kinematics block: T_dyn, v and vj, laid out in that order
         * by rd_state_init. Past it is scratch that the algorithms above have
         * written in `state` and never in `cold`. */
        size_t floats = (size_t)chain.n_nodes * (16 + 6 + 1);
        int differing = 0;
